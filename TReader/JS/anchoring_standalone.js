...
function attach_annotation(exact, prefix) {
  ...
  highlight.id = 'annontation-1';
  highlight.setAttribute('data', 'data');
  highlight.title = "anno";
  highlight.className = 'hypothesis_annotation';

  wrap(highlight, range);

  const _ra = JSON.stringify(range);
  const _hi = JSON.stringify(highlight);
  const message = {
    exact: exact,
    prefix: prefix,
    range: _ra,
